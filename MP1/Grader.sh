...
joincount=`grep joined dbg.log | uniq | wc -l`
...
failcount=`grep removed dbg.log | uniq | wc -l`
if [ $failcount -eq 9 ]; then
	grade=`expr $grade + 10`
...
accuracycount=`grep removed dbg.log | uniq | grep $failednode | wc -l`
if [ $accuracycount -eq 9 ]; then
	grade=`expr $grade + 10`
...
for i in $failednode
do
	accuracycount=`grep removed dbg.log | uniq | grep $i | wc -l`
	if [ $accuracycount -eq 5 ]; then
		tmp=`expr $tmp + 2`
		grade=`expr $grade + 2`
	fi
done
