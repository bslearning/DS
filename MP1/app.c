...
int main(int argc, char *argv[]){
		...
		for(i=0;i<=EN_GPSZ-1;i++) {
				printf("inited for %d",i);
			//	printf(" : value of bfailed  %d",group[i].bfailed);
			//	printf("\n");
				group[i].inited=0;
		}


		for(globaltime=0; globaltime<50; ++globaltime) {
				//sleep(1);  	 
				printf(">>>>>>>>>>> GLOBAL TIME >>>>>>>>>>>> : %d\n",globaltime);
				/* call recvloop for all nodes currently in the system */
				for(i=0;i<=EN_GPSZ-1;i++){
						//printf("|__ getCurrentime :%d STEP_RATE*i:%d group[i].bfailed:%d",getcurrtime(),STEP_RATE*i,group[i].bfailed);
						if(getcurrtime()>(int)(STEP_RATE*i) && group[i].bfailed==0){
				//				printf("inside RECVLOOP for node : %d",i);
								recvloop(&group[i]);
				//				printf("\n");
						}
				}
				...
				//printf(" EN_GPZ LOOP %d",i);
				//printf("\n");
						//printf("|__ getCurrentime :%d STEP_RATE*i:%d i:%d",getcurrtime(),STEP_RATE*i,i);
						if(getcurrtime() == (int)(STEP_RATE*i)) {
								/* introduce the ith node into the system at time STEPRATE*i */
								//printf("inside NODESTART");
								//printf("\n");
								...
								//printf("inside NODELOOP : %d",i);
								//printf("\n");
								nodeloop(&group[i]);
								//printf("\n");
								//printf("done NODELOOP : %d",i);
								//printf("\n");
